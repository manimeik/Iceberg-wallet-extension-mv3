...
import NotWhiteListed from './not-white-listed/not-white-listed';
...
export default class PermissionConnect extends Component {
  ...
  componentDidMount() {
    ...
    if (whiteList.includes(targetSubjectMetadata?.name)) {
  ...
  renderTopBar() {
    const { redirecting, whiteListed } = this.state;
    const { page, isRequestingAccounts, totalPages } = this.props;
    const { t } = this.context;
    return redirecting || !whiteListed ? null : (
  ...
  handleConfirm = () => {
    ...
    setWhiteList([...whiteList, targetSubjectMetadata?.name]);
    this.setState({ whiteListed: true });
  };
    ...
    return (
      <div className="permissions-connect">
        ...
        {whiteListed ? (
          ...
          <NotWhiteListed
            targetDomainMetadata={targetSubjectMetadata}
            cancelPermissionsRequest={(requestId) =>
              this.cancelPermissionsRequest(requestId)
            }
            permissionsRequestId={permissionsRequestId}
